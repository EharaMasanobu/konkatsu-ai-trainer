...
OpenAI API キーは **コードに書かず**、Next.js 標準の `process.env.OPENAI_API_KEY` として読み込みます。  
ローカルは `.env.local`、Railway は **Variables** から自動注入されます。

### 変数一覧

| 変数名 | 必須 | ローカル | Railway | 説明 |
| --- | --- | --- | --- | --- |
...
`.env.local` を編集し、API キーを設定します（**引用符なし推奨**）。
...
- 変更後は `npm run dev` を再起動

### Railway: Variables の設定

1. Railway ダッシュボード → 対象 **Service** → **Variables**
2. **New Variable** をクリック
3. 以下を追加:

| Name | Value |
| --- | --- |
| `OPENAI_API_KEY` | `sk-...`（本番用キー） |
| `DATABASE_URL` | `file:./prisma/prod.db`（任意） |

4. **Deploy** を再実行（Variables 変更後は再デプロイが必要）

> 変数名は大文字の `OPENAI_API_KEY` と完全一致させてください。スペースや引用符を Value に含めないでください。

### 動作確認

#### 1. ヘルスチェック（プロセス生存 + キー設定確認）
...
| `openaiConfigured` | 意味 |
| --- | --- |
| `true` | `OPENAI_API_KEY` が設定済み |
| `false` | 未設定（`/api/health` は常に HTTP 200） |

#### 2. システムチェック（OpenAI 必須の詳細確認）

```bash
curl https://<your-app>.up.railway.app/api/system/check
```

`OPENAI_API_KEY` 未設定時は HTTP `503`。

#### 3. 会話 API

ブラウザで会話を1ターン送信し、AI が返答することを確認します。

#### 4. 起動ログ

キー未設定でサーバー起動時、ログに以下が出力されます:
...
- OpenAI クライアント: `src/ai/OpenAIClient.ts` / `src/ai/WhisperClient.ts`
- キー参照: `process.env.OPENAI_API_KEY`（Next.js が `.env.local` / Railway Variables から注入）
- 起動時チェック: `src/instrumentation.ts`
