...
export class RomanceScoreCalculator {
  computeUpdate(
    ...
    if (maleAnalysis) {
      const maleBonus = this.computeMaleAnalysisBonus(maleAnalysis, reasons);
      delta += maleBonus;
    }
  ...
  private computeMaleAnalysisBonus(
    ...
    if (analysis.topicDepth) {
      bonus += 2;
      reasons.push("話題を深掘りできた");
    }
    if (analysis.backchannelOnly) {
      bonus -= 3;
      reasons.push("会話が続かなかった");
    }
