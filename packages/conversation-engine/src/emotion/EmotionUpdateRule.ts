...
import type { FemaleEmotionKey, FemaleEmotionState } from "@engine/emotion/FemaleEmotionState";
import { clampEmotion } from "@engine/emotion/FemaleEmotionState";
...
export class EmotionUpdateRule {
  evaluate(context: EmotionUpdateContext): EmotionChangeEntry[] {
    ...
    if (context.maleAnalysis) {
      changes.push(...this.evaluateFromMaleAnalysis(context.maleAnalysis));
    }

    return changes;
  }

  private evaluateFromMaleAnalysis(
    ...
    if (analysis.topicDepth) {
      changes.push({
        field: "interest",
        delta: 3,
        reason: "話題を深掘りしてくれたため",
      });
    }

    if (analysis.naturalReaction && !analysis.backchannelOnly) {
      changes.push({
        field: "comfort",
        delta: 3,
        reason: "自然なリアクションがあったため",
      });
    }
